CREATE OR REPLACE TRIGGER C_INT_CONSUMER_ENDERECO_TRG
   FOR INSERT OR UPDATE OR DELETE ON CUSTOMER_INFO_ADDRESS_TAB
   COMPOUND TRIGGER
      -- local variables here
      msg_erro_   VARCHAR2(2000);
      linha       NUMBER;

      TYPE rec_type IS RECORD (codigoCliente_ CUSTOMER_INFO_ADDRESS_TAB.CUSTOMER_ID%TYPE,
      ...
      AFTER EACH ROW IS
         ...
         EXCEPTION
             WHEN OTHERS THEN
                msg_erro_ := 'ERROR  TRIGGER - C_INT_CONSUMER_ENDERECO_TRG: '|| substr(sqlerrm,1,1600);
      ...
      AFTER STATEMENT IS
         ...
         EXCEPTION
             WHEN OTHERS THEN
                ...
                C_INT_MAGENTO_CONSU_UTIL_API.GRAVAR_LOG_TRIGGER(USER, msg_erro_); 
      END AFTER STATEMENT;
END C_INT_CONSUMER_ENDERECO_TRG;
